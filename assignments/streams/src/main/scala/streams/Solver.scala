...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    neighbors.toList match {
      ...
      case head :: tail =>
        if (explored.contains(head._1)) newNeighborsOnly(tail.toStream, explored)
        else head #:: newNeighborsOnly(tail.toStream, explored + head._1)
    }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      if (!neighbors.isEmpty) initial #::: from(neighbors, explored ++ neighbors.map(_._1))
      else initial
    }
